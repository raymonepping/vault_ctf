<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-50">
    <div class="max-w-xl w-full p-8 rounded-2xl bg-white shadow-2xl border border-gray-100">
      ...
      <h2 class="text-2xl font-bold mb-4">Level 5: 🎭 Shaper of Identities</h2>
...
const validateLevel = async () => {
  ...
  try {
    ...
    const res = await $fetch('/api/validate', {
      method: 'POST',
      body: { username, level: 5 }
    })
    if (res.success) {
      message.value = 'Level 5 complete! 🎉 Proceeding to Level 6...'
      validationStatus.value = 'success'
      setTimeout(() => {
        router.push('/levels/6')
      }, 1500)
    } else {
