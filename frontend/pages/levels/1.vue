<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-50">
    <div class="max-w-xl w-full p-8 rounded-2xl bg-white shadow-2xl border border-gray-100">
      ...
      <h2 class="text-2xl font-bold mb-4">Level 1: Defender of the Gate</h2>
      ...
      <div class="mb-6">Keys Left: <span class="font-mono">{{ player?.keysLeft }}</span></div>
...
onMounted(async () => {
  ...
  try {
    ...
    player.value = { username: 'unknown', currentLevel: 1, keysLeft: 3 }
  }
...
const validateLevel = async () => {
  ...
  try {
    ...
    if (res.success) {
      message.value = 'Level 1 complete! 🎉 Proceed to the next level.'
